...
def imshow_with_histogram(image, **kwargs):
    ...
    if (image.dtype == "uint8"):
        ax_image.imshow(image, norm=colors.Normalize(vmin=0, vmax=255), **kwargs)
    elif(image.dtype == "float" or image.dtype == "float64" or image.dtype == "float32"):
        ax_image.imshow(image, norm=colors.Normalize(vmin=0, vmax=1), **kwargs)
    else:
